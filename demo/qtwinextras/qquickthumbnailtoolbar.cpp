...
QQuickThumbnailToolBar::QQuickThumbnailToolBar(QQuickItem *parent) : QQuickItem(parent)
{
    m_toolbar.reset(new QWinThumbnailToolBar);
    connect(m_toolbar.data(), &QWinThumbnailToolBar::iconicThumbnailPixmapRequested,
            this, &QQuickThumbnailToolBar::iconicThumbnailRequested);
    connect(m_toolbar.data(), &QWinThumbnailToolBar::iconicLivePreviewPixmapRequested,
...
void QQuickThumbnailToolBar::setIconicNotificationsEnabled(const bool enabled)
{
    if (enabled != m_toolbar->iconicPixmapNotificationsEnabled()) {
        m_toolbar->setIconicPixmapNotificationsEnabled(enabled);
        Q_EMIT iconicNotificationsEnabledChanged();
    }
}

void QQuickThumbnailToolBar::iconicThumbnailLoaded(const QVariant &value)
{
    m_toolbar->setIconicThumbnailPixmap(qvariant_cast<QPixmap>(value));
}
...
void QQuickThumbnailToolBar::setIconicThumbnailSource(const QUrl &source)
{
    ...
    if (source.isEmpty()) {
        ...
        Q_EMIT iconicThumbnailSourceChanged();
    }

    if (QQuickIconLoader::load(source, qmlEngine(this), QMetaType::QPixmap, QSize(),
                               this, &QQuickThumbnailToolBar::iconicThumbnailLoaded) != QQuickIconLoader::LoadResult::Error) {
        m_iconicThumbnailSource = source;
        Q_EMIT iconicThumbnailSourceChanged();
    }
}
...
{
    m_toolbar->setIconicLivePreviewPixmap(qvariant_cast<QPixmap>(value));
}
...
void QQuickThumbnailToolBar::setIconicLivePreviewSource(const QUrl &source)
{
    ...
    if (source.isEmpty()) {
        m_toolbar->setIconicLivePreviewPixmap(QPixmap());
         ...
         Q_EMIT iconicLivePreviewSourceChanged();
    }

    if (QQuickIconLoader::load(source, qmlEngine(this), QMetaType::QPixmap, QSize(),
                               this, &QQuickThumbnailToolBar::iconicLivePreviewLoaded) != QQuickIconLoader::LoadResult::Error) {
        m_iconicLivePreviewSource = source;
        Q_EMIT iconicLivePreviewSourceChanged();
    }
}

void QQuickThumbnailToolBar::itemChange(const ItemChange change, const ItemChangeData &data)
{
...
void QQuickThumbnailToolBar::addData(QQmlListProperty<QObject> *property, QObject *object)
{
    if (auto *button = qobject_cast<QQuickThumbnailToolButton *>(object)) {
        ...
        Q_EMIT quickThumbbar->countChanged();
        Q_EMIT quickThumbbar->buttonsChanged();
    }
}
